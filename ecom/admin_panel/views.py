...
def admin_dashboard(request):
    ...
    active_user = Account.objects.filter(
        Q(is_active=True) & Q(is_admin=False)).count()
    blocked_user = Account.objects.filter(
        Q(is_active=False) & Q(is_admin=False)).count()

    total_orders = Order.objects.all().count()
    pay = Payment.objects.values('payment_method').annotate(
    ...
    status = Order.objects.filter(is_ordered=True).values(
    ...
    try:
        ...
        order_product_count_graph = OrderProduct.objects.filter().values(
            'created_at__date').order_by('-created_at__date')[:day].annotate(count=Count('quantity'))
    except:
        order_product_count_graph = OrderProduct.objects.filter().values(
            'created_at__date').order_by('-created_at__date')[:7].annotate(count=Count('quantity'))
...
def update_category(request, id):
    if request.method == "POST":
        ...
        categ = category(id=id, category_name=category_name,
...
def update_brand(request, id):
    if request.method == "POST":
        ...
        brandd = brand(id=id, brand_name=brand_name,
                       slug=slug,  description=description)
        brandd.save()
        return redirect('admin_brand')
    return render(request, 'admin/admin_brand.html')
...
def admin_product(request):
    ...
    context = {'products': paged_products, 'brands': brands,
    ...
    if request.method == 'POST':
        if len(request.FILES) != 0:
            if product_detail.images:
                product_detail.images = request.FILES.get('image1')
...
def update_variation(request, id):
    ...
    if request.method == "POST":
        ...
        variation_detail.variation_category = request.POST.get(
            'variation_category')
    ...
    if request.method == "POST":
        ...
        if len(subcategory_name.strip()) == 0:
            ...
            subcategory = SubCategory(
                category_id=category_id,  sub_category_name=subcategory_name, slug=slug)
            subcategory.save()
            return JsonResponse({
                'success': True},
                safe=False)
...
def update_subcategory(request, id):
    if request.method == "POST":
        ...
        subcategory.slug = subcategory.subcategory_name.replace(
            " ", "-").lower()
...
def admin_order(request):
    ...
    orders_pending = Order.objects.filter(
        status__contains='New', is_ordered=True).count()
    orders_deliverd = Order.objects.filter(
        status__contains='Completed').count()
        ...
        try:
            ...
            if discount < 0 or discount > 80:
                ...
                coupons = Coupon.objects.create(
                    coupon_code=coupon_code, disccount=discount, minimum_amount=minimum_amount, maximum_amount=maximum_amount)
                coupons.save()
                return JsonResponse(
...
def admin_sales(request, *args, **kwargs):
    ...
    if request.method == "POST":
        try:
            ...
            sales = OrderProduct.objects.filter(created_at__date=salesdate).values(
        ...
        try:
            ...
            sales = OrderProduct.objects.filter().values(
                'product_id').annotate(qty=Sum('quantity'))
...
def export_excel(request):
    ...
    rows = Order.objects.filter(Q(created_at__date=salesdate) & Q(is_ordered=True)).values_list(
        'order_number', 'first_name', 'order_total', 'tax', 'created_at__date')
