...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        if request.method == 'POST':
            for item in request.POST:
                ...
                try:
                    varia = Variation.objects.get(
                        product=prod, variation_category__iexact=key, variation_value__iexact=value)
                    product_variation.append(varia)
                except:
                    pass

        is_cart_item_exists = CartItem.objects.filter(
            product=prod, user=current_user).exists()
        if is_cart_item_exists:

            cart_item = CartItem.objects.filter(
                product=prod, user=current_user)
            ...
            if product_variation in existing_variation_list:
                ...
                item = CartItem.objects.create(
                    product=prod, quantity=1, user=current_user)
   ...
   # if the user is not auth
        ...
        try:
            ...
            # get the cart id by using the session
        ...
        if is_cart_item_exists:
            ...
            if product_variation in existing_variation_list:
                ...
                # create a new cart item
                item = CartItem.objects.create(
                    product=prod, quantity=1, cart=cart)
                if len(product_variation) > 0:
                    item.variation.clear()
                    item.variation.add(*product_variation)
                    item.save()
...
def cart(request, total=0, quantity=0, cart_items=None):
    ...
    try:
        ...
        if request.user.is_authenticated:
            ...
            try:
                cart_ii = CartItem.objects.values().filter(
        ...
        for cart_item in cart_items:
            if cart_item.product.discount > cart_item.product.category.discount:
                total += (int(cart_item.product.price - (cart_item.product.price *
                          cart_item.product.discount * 0.01)) * cart_item.quantity)
            else:
                total += (int(cart_item.product.price - (cart_item.product.price *
                          cart_item.product.category.discount * 0.01)) * cart_item.quantity)
    ...
    try:
        if request.user.is_authenticated:
            cart_item = CartItem.objects.get(
                product=prod, user=request.user, id=cart_item_id)
        else:
            cart = Cart.objects.get(cart_id=_cart_id(request))
            cart_item = CartItem.objects.get(
                product=prod, cart=cart, id=cart_item_id)
...
def checkout(request, totals=0, quantity=0, cart_items=None):
    ...
    if request.user.is_authenticated:
        ...
        addressesfirst = Address.objects.filter(
            user=request.user).order_by('id')[:1]
        current_user = request.user
        us = current_user.id
        ...
        cart_items = CartItem.objects.filter(cart=cart, is_active=True)
        cart_ii = CartItem.objects.values().filter(
            cart=cart, is_active=True).order_by('id')[:1]
    for cart_item in cart_items:
        if cart_item.product.discount > cart_item.product.category.discount:
            totals += (int(cart_item.product.price - (cart_item.product.price *
                       cart_item.product.discount * 0.01)) * cart_item.quantity)
        else:
            totals += (int(cart_item.product.price - (cart_item.product.price *
                       cart_item.product.category.discount * 0.01)) * cart_item.quantity)
